<?php

$L = array(

'site_settings_heading' => 'BucketList Settings',
'access_key_id'         => 'Access Key ID',
'secret_access_key'     => 'Secret Access Key',
'custom_url'            => 'Custom URL',
'custom_url_hint'       => "If you've configured a sub-domain to point at your Amazon S3 account, you can enter the URL here.",

'use_ssl' => 'Use SSL?',
'yes'     => 'Yes',
'no'      => 'No',

'cache_duration'        => 'Cache duration',
'cache_duration_hint'   => "Caching the list of your Amazon S3 files results in fewer queries and lower costs.",
'5_min'                 => '5 minutes',
'10_min'                => '10 minutes',
'15_min'                => '15 minutes',
'30_min'                => '30 minutes',
'45_min'                => '45 minutes',
'60_min'                => '1 hour',
'90_min'                => '1.5 hours',
'120_min'               => '2 hours',
'240_min'               => '4 hours',
'360_min'               => '6 hours',
'480_min'               => '8 hours',

'field_settings_heading'  => 'BucketList Field Settings',
'buckets_heading'         => 'Select "Buckets"',
'buckets_info'            => 'Select the Amazon S3 buckets that you wish to make available for this field',
'buckets_label'           => 'Buckets',

'buckets'         => 'Select Folder',
'files'           => 'Select File',

'missing_credentials'     => 'Please enter your Amazon S3 Access Key ID and Secret Access Key on the extension settings screen',
'no_buckets'              => 'There are no available buckets for this account.',
'no_available_buckets'    => 'There are no available folders for this field.',

'loading_files'         => 'Loading Files',
'error_loading_files'   => 'Unable to Load Files',
'select_bucket_to_load_files'   => 'Select a folder from the list to display its associated files.',

// All done.
'' => ''

);

?>